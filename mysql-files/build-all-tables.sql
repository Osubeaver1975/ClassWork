...
-- You can pretty much ALTER anything, add cols, remove cols, add/remove primary or foreign keys et al
...
insert into User
  (name, phone, email)   -- uid column is not included because it is automagically generated
...
create table Customer (
    ...
    address varchar(256),
    -- (col-in-this-table)    parent-table (col-in-parent) -- should be the primary key of parent table
...
-- Customer is a dependent of User table! ! ! 
--     The uid  col in Customer table must matcjh an existing uid in the User Table
-- the uid in the User table is automagically generated - we don't know what that 
--       value is for a specific user
--   BUT the DBMS knows the uid fore each user
-- so we wil ask the DBMS for uid in User for a specific user with a SELECT on the INSERT 
-- instead of values()

insert into Customer
  (uid, address)
    -- retrieve uid, 'use this literal' from User table  where name is Frank
  select uid, '1 Java Ln, Phoenix AZ 85339' from User where name ='Frank'
;

-- LIKE does a wildcard search of a table
-- 
...
insert into Customer
  (uid, address)
  select uid, '1 Java Ln, Phoenix AZ 85339' from User where name like 'tink%' -- Stars with 'tink'
;

insert into Customer
  (uid, address)
  select uid, 'Death Star' from User where name like '%Vader%' -- contains 'Vader'
;
