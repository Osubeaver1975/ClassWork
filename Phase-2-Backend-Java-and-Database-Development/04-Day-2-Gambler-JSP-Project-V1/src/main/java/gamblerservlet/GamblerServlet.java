...
// Determine processing based on the query parameter(s) passed to the servlet	
		...
		// Handle a delete Gambler request
		
		if(id2Delete != null) {
			// call the DAO method with the Id of the gmabler to delete
			// Note: We have to convert the query param from String to Long to send to the DAO method
			try {
			gamblerTable.delete(Long.parseLong(id2Delete));
		} catch (NumberFormatException | DataBaseDeleteException  exceptionBlock) {
			databaseLog.writeToDatabaseErrorLog("Error processing the id to be deleted");
			databaseLog.writeExceptionInfoToDatabaseErrorLog(exceptionBlock);
		}
